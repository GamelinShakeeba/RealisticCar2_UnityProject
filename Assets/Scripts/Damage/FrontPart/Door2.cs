...
{
    public int countToBreak2 = 0;
    public float detachForce2 = 30f;
    ...
    public void SetupHingeJoint2()
    {
        ...
        hingeJoint2 = targetObject2.AddComponent<HingeJoint>();
        hingeJoint2.anchor = Vector3.zero; // Adjust as needed
        hingeJoint2.axis = Vector3.up; // Adjust as needed
        hingeJoint2.useLimits = true;
        JointLimits limits = hingeJoint2.limits;
        limits.min = -90;
        limits.max = 90;
        hingeJoint2.limits = limits;
        hingeJoint2.useMotor = false;

        BoxCollider boxCollider = targetObject2.AddComponent<BoxCollider>();
        boxCollider.isTrigger = false;

        // Find the Rigidbody of the grandparent
        Transform grandparentTransform = targetObject2.transform.parent?.parent?.parent;
        if (grandparentTransform != null)
        {
            ...
            if (grandparentRigidbody != null)
            {
                ...
                hingeJoint2.connectedBody = grandparentRigidbody;
            }
        }

        hingeJoint2.anchor = Vector3.zero; // Local anchor point
        hingeJoint2.axis = Vector3.up; // Axis of rotation
    }

    public void DetachedDoor2()
    {
        Destroy(hingeJoint2);
        transform.position += Vector3.down * detachForce2 * Time.deltaTime;
        StartCoroutine(DestroyDoor2());
    }

    IEnumerator DestroyDoor2()
    {
