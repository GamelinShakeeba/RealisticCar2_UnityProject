...
{
    public int countToBreak = 0;
    public float detachForce = 30f;
    ...
    public void UpdateMeshBreaking()
    {
        for (int i = 0; i < m_oVertices.Length; i++)
        {
    ...
    public void SetupHingeJoint()
    {
        ...
        // Add the HingeJoint component to the target object
        hingeJoint = targetObject.AddComponent<HingeJoint>();
        hingeJoint.anchor = Vector3.zero; // Adjust as needed
        hingeJoint.axis = Vector3.up; // Adjust as needed
        ...
        // Find the Rigidbody of the grandparent
        Transform grandparentTransform = targetObject.transform.parent?.parent?.parent;
        if (grandparentTransform != null)
        {
            Rigidbody grandparentRigidbody = grandparentTransform.GetComponent<Rigidbody>();
            if (grandparentRigidbody != null)
            {
                // Set the connected body
                hingeJoint.connectedBody = grandparentRigidbody;
            }
            else
            {
                Debug.LogError("Rigidbody not found on the grandparent.");
            }
        }
        else
        {
            Debug.LogError("Grandparent is missing.");
        }

        // Optional: Configure hinge joint properties
        hingeJoint.anchor = Vector3.zero; // Local anchor point
        hingeJoint.axis = Vector3.up; // Axis of rotation
    }

    public void DetachedDoor1()
    {
        Destroy(hingeJoint);
        transform.position += Vector3.down * detachForce * Time.deltaTime;
        StartCoroutine(DestroyDoor1());
    }

    IEnumerator DestroyDoor1()
    {
